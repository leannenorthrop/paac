...
protected trait Year2015Period2Calculator extends PeriodCalculator {
  ...
  protected lazy val _aaCCF =
    if (!isTriggered) {
      // TODO add this proposed fix by linda however it appears to be invalid
      // val p2013 = previousPeriods.find(isYear(contribution.taxPeriodStart.taxYear-3)).map(_.summaryResult.unusedAllowance).getOrElse(0L)
      // val v = (period1.availableAAWithCCF - basicDefinedBenefit - p2013).max(0)
      val v = (period1.availableAAWithCCF - basicDefinedBenefit).max(0)
      Logger.debug(s"AACCF (nte): ${period1.availableAAWithCCF} - ${basicDefinedBenefit} = ${v}")
      v
    } else {
