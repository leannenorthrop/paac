  ...
  "Year 2015 Period 1 Calculator" should {
    ...
    "return some results for contributions other than 2015 period 1" in {
      (0 until 94).foreach {
        ...
        // check it
        withClue(s"Contributions with date '$taxDay/${taxMonth+1}/$taxYear' should be supported but") { results shouldBe Some(ExtendedSummaryFields(0,0,8000000,4000000,8000000,4000000,0,0,0,0,0,0,0,0,0,5000,0,0,5000,0,false,5995000,0)) }
      }
    }
    ...
    "when no previous allowance available" can {
      "return expected summary results when no previous entries supplied and 0 defined benefit is given" in {
        ...
        // check it
        results shouldBe Some(ExtendedSummaryFields(0,0,8000000,4000000,8000000,4000000,acaCF=6000000))
      }
